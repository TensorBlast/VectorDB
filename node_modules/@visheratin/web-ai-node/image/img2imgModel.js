 ...
 */
class Img2ImgModel extends base_js_1.BaseImageModel {
    constructor() {
        super(...arguments);
        /**
         ...
         */
        this.process = async (input, resize = 0) => {
            if (!this.initialized || !this.preprocessor) {
                throw Error("the model is not initialized");
            }
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            let image = await jimp_1.default.read(input);
            if (resize > 0) {
            ...
            if (this.preprocessor &&
                this.preprocessor.config &&
                this.preprocessor.config.pad) {
            ...
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    ...
                    let value = output.data[tensorIdx];
                    if (value < 0) {
                        value = 0;
                    }
                    else {
                    ...
                    value *= 255.0;
                    pixels[pixIdx] = value;
                    value = output.data[size + tensorIdx];
                    if (value < 0) {
                        value = 0;
                    }
                    else {
                    ...
                    value = output.data[2 * size + tensorIdx];
                    if (value < 0) {
                        value = 0;
                    }
                    else {
        ...
        this.prepareImage = (image, resize) => {
...
exports.Img2ImgModel = Img2ImgModel;
//# sourceMappingURL=img2imgModel.js.map